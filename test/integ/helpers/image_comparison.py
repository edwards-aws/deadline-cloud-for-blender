...
def assert_all_images_close(expected_image_directory: Path, actual_image_directory):
    for image in (expected_image_directory).iterdir():
        ...
        # with tolerance.
        actual_location = actual_image_directory / image.name
        actual = np.asarray(PIL.Image.open(actual_location))
        ...
        assert np.allclose(actual, expected, atol=21), f"Failed image: {actual_location}"
